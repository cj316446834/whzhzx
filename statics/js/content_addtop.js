...
//移除相关案例
function remove_relation_dbal(sid,id) {
	var relation_ids = $('#xm_dbal').val();
	if(relation_ids !='' ) {
		$('#'+sid).remove();
		var r_arr = relation_ids.split('|');
		var newrelation_ids = '';
		...
		$('#xm_dbal').val(newrelation_ids);
	}
}
//显示相关案例
function show_relation_dbal(modelid,id) {
$.getJSON("?m=content&c=content&a=public_getjson_ids&modelid="+modelid+"&id="+id, function(json){
	var newrelation_ids = '';
	if(json==null) {
		alert('没有添加对比案例');
	...
	$('#relation_dbal_text').html(newrelation_ids);
}); 
}
//移除ID
